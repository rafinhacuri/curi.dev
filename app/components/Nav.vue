    ...
    <div class="mx-auto flex max-w-7xl flex-wrap items-center justify-between p-4">
      ...
      <div class="flex flex-col items-center sm:mt-0 lg:order-2 lg:flex-row">
        <div class="flex items-center space-x-4 sm:mt-0 md:ml-16 lg:order-2">
          <NuxtLink v-for="{icon, link, name} of socials" :key="name" :to="link" target="_blank" :aria-label="name" class="group relative text-lg font-medium text-gray-400 transition-all duration-300 ease-in-out before:absolute before:-bottom-1 before:left-1/2 before:h-[3px] before:w-0 before:-translate-x-1/2 before:bg-yellow-400 before:transition-all before:duration-500 before:ease-out hover:scale-105 hover:text-yellow-400 hover:drop-shadow-[0_0_5px_#F28AA9] hover:before:w-full">
            <Icon :name="icon" class="text-gray-400 group-hover:text-yellow-400" size="30" />
          </NuxtLink>
          <button :aria-label="t('nav.language')" class="group relative text-lg font-medium text-gray-400 transition-all duration-300 ease-in-out before:absolute before:-bottom-1 before:left-1/2 before:h-[3px] before:w-0 before:-translate-x-1/2 before:bg-yellow-400 before:transition-all before:duration-500 before:ease-out hover:scale-105 hover:text-yellow-400 hover:drop-shadow-[0_0_5px_#F28AA9] hover:before:w-full" @click="changeLanguage">
            <Icon name="ion:language-outline" class="text-gray-400 group-hover:text-yellow-400" size="30" />
          </button>
          <button :aria-label="t('nav.menu')" class=" lg:hidden" @click="menuClosed = !menuClosed">
            <Icon :name="!menuClosed ? 'lucide:list-x' : 'lucide:menu'" class="text-gray-400 " size="30" />
          </button>
        </div>
